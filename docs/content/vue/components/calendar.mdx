...
<DocPreview framework="vue" slug="calendar" kind="component"/>
...
const notice = ref('');
const calendarDate = ref<string | { start: string; end: string } | null>("2026-09-17");
</script>

<template>
  <Calendar v-model="calendarDate" @value-change="notice = `${$event} selected`" />
